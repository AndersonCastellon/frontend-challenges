...
function slideNext$1(speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this;
  const {
    enabled,
    params,
...
/* eslint no-unused-vars: "off" */
function slidePrev$1(speed = this.params.speed, runCallbacks = true, internal) {
  const swiper = this;
  const {
    params,
    snapGrid,
...
function getPlanInfo(planId, yearly) {
...
function setPlanInfo(planInfo, element, yearly) {
  const costElement = element.querySelector("#plan-cost");
  const freeElement = element.querySelector("#free-months");
  if (costElement) {
    const cost = planInfo.cost || 0;
    costElement.innerText = `${cost}/${yearly ? "yr" : "mo"}`;
  }
  if (freeElement) {
    const freeMonths = planInfo.freeMonths || 0;
    if (freeMonths > 0) {
